...
// Сколько щелчков энкодера должно уходить на весь диапазон громкости (0 -> 100%)
constexpr int kVolumeEncoderClicksToMax = 50;
// "Вес" одного щелчка энкодера в единицах громкости (0..254), округлено вверх,
// чтобы 100% гарантированно достигалось не позже, чем за kVolumeEncoderClicksToMax щелчков
constexpr int kVolumeStepSize =
    (kVolumePercentMax - kVolumePercentMin + kVolumeEncoderClicksToMax - 1) / kVolumeEncoderClicksToMax;

void handleVolumeStep(int delta) {
  noteUserActivity();

  // Просто прибавляем дельту энкодера (с учётом веса шага) к текущей громкости.
  // НЕ синхронизируем позицию энкодера с currentVolume на каждом шаге -
  // encoderChanged() и так корректно считает дельту независимо от абсолютной
  // позиции. Постоянный резинк только усиливал контактный дребезг при
  // развороте направления вращения.
  const int newVolume = constrain(currentVolume + delta * kVolumeStepSize,
                                   kVolumePercentMin,
                                   kVolumePercentMax);
...
void setup() {
  ...
  // loop() (и, значит, audio.loop()) должен иметь приоритет ВЫШЕ displayTask (2),
  // иначе FreeRTOS всегда отдаёт CPU дисплею в первую очередь и аудио заикается
...
void loop() {
    // Аудио должно обслуживаться первым и как можно чаще —
    // это критично для плавного стриминга без заиканий
    ...
    // audio.loop() уже вызван в начале loop() выше
