...
extern int fftWindowX, fftWindowY, fftWindowW, fftWindowH;
extern int fftBands, fftSegments, fftMinBin, fftMaxBin, fftAggregation, fftCurve, fftProfile;
extern float fftGain, fftPreGain, fftNoiseGate, fftNoiseFloor, fftSensitivity, fftGamma, fftLogStrength;
extern float fftAttack, fftRelease, fftLowBoost, fftMidBoost, fftHighBoost;
extern int fftBarGap, fftSideMargin, fftTopMargin, fftBottomMargin, fftMinLitSegments, fftPeakFall;
extern bool fftPeakHold, fftMirror, fftInvert;
...
extern uint16_t vuBarsColorBottom, vuBarsColorTop;
extern uint16_t fftColorBottom, fftColorTop, fftColorPeak;
...
bool g_archiveImportInProgress = false;
bool g_uploadHadError = false;
String g_pendingTarPath;
String g_uploadResponseMessage;

struct ArchiveImportRuntimeState {
  bool radioWasStarted = false;
  bool radioWasPaused = false;
};

ArchiveImportRuntimeState g_archiveImportRuntimeState;

int nonNegativeCoord(int value) {
  return value < 0 ? 0 : value;
}
bool g_archiveImportRuntimeStateValid = false;

struct ParsedPlaylistEntry {
  String name;
  String url;
};
...
String trimBom(const String& input) {
  String out = input;
  out.trim();
  if (out.length() >= 3
...
String sanitizeImportedPlaylistField(String value) {
  value = trimBom(value);
  value.replace("\n", " ");
  value.replace("\r", " ");
  value.replace("\t", " ");
...
bool isHttpStreamUrl(const String& value) {
  return value.startsWith("http://") || value.startsWith("https://");
}

String defaultStationName(size_t index) {
  return String("Station ") + String(index + 1);
}

void appendPlaylistEntry(std::vector<ParsedPlaylistEntry>& entries, const String& rawName, const String& rawUrl) {
  const String url = sanitizeImportedPlaylistField(rawUrl);
  if (!isHttpStreamUrl(url)) return;

  ParsedPlaylistEntry entry;
  entry.url = url;
  entry.name = sanitizeImportedPlaylistField(rawName);
  if (!entry.name.length()) entry.name = defaultStationName(entries.size());
  entries.push_back(entry);
}

void ensureVectorSize(std::vector<String>& values, size_t index) {
  if (values.size() <= index) values.resize(index + 1);
}

bool parsePlaylistDelimitedLine(const String& line, std::vector<ParsedPlaylistEntry>& entries) {
  String trimmed = trimBom(line);
  trimmed.trim();
  if (!trimmed.length() || trimmed.startsWith("#")) return false;

  if (isHttpStreamUrl(trimmed)) {
    appendPlaylistEntry(entries, String(), trimmed);
    return true;
  }
  ...
  if (trimmed.indexOf("\",\"") >= 0) {
    ...
    while (start <= trimmed.length()) {
      ...
      if (pos < 0) {
        fields.push_back(trimmed.substring(start));
        break;
      }
      fields.push_back(trimmed.substring(start, pos));
      start = pos + 3;
    }
  } else {
    char separator = 0;
    if (trimmed.indexOf(';') >= 0) separator = ';';
    else if (trimmed.indexOf('\t') >= 0) separator = '\t';
    else if (trimmed.indexOf(',') >= 0) separator = ',';
    if (!separator) return false;

    int start = 0;
    while (start <= trimmed.length()) {
      int pos = trimmed.indexOf(separator, start);
      if (pos < 0) {
        fields.push_back(trimmed.substring(start));
        break;
      }
  ...
  appendPlaylistEntry(entries, fields[0], fields[1]);
  return true;
}

bool parseM3UPlaylist(File& file, std::vector<ParsedPlaylistEntry>& entries) {
  String pendingName;
  while (file.available()) {
    String line = trimBom(file.readStringUntil('\n'));
    line.trim();
    if (!line.length()) continue;

    if (line.startsWith("#EXTINF:")) {
      const int commaPos = line.indexOf(',');
      pendingName = commaPos >= 0 ? line.substring(commaPos + 1) : String();
      pendingName = sanitizeImportedPlaylistField(pendingName);
      continue;
    }

    if (line.startsWith("#")) continue;
    if (isHttpStreamUrl(line)) {
      appendPlaylistEntry(entries, pendingName, line);
      pendingName = "";
    }
  }
...
bool parsePLSPlaylist(File& file, std::vector<ParsedPlaylistEntry>& entries) {
  ...
  while (file.available()) {
    ...
    if (key.startsWith("File")) {
      ...
      if (idx > 0) {
        ensureVectorSize(files, static_cast<size_t>(idx));
        files[idx] = value;
      }
    } else if (key.startsWith("Title")) {
      int idx = key.substring(5).toInt();
      if (idx > 0) {
        ensureVectorSize(titles, static_cast<size_t>(idx));
        titles[idx] = value;
      }
    }
  }

  const size_t count = files.size() > titles.size() ? files.size() : titles.size();
  for (size_t i = 1; i < count; ++i) {
    if (!files[i].length()) continue;
    appendPlaylistEntry(entries, i < titles.size() ? titles[i] : String(), files[i]);
  }
  return !entries.empty();
}

bool parseTextLikePlaylist(File& file, std::vector<ParsedPlaylistEntry>& entries) {
  while (file.available()) {
    parsePlaylistDelimitedLine(file.readStringUntil('\n'), entries);
  }
  return !entries.empty();
}

bool parseJsonPlaylist(const String& content, std::vector<ParsedPlaylistEntry>& entries) {
  JsonDocument doc;
  ...
  } else if (doc.is<JsonObject>() && doc["stations"].is<JsonArray>()) {
    array = doc["stations"].as<JsonArray>();
  } else {
    return false;
  }
  ...
  for (JsonVariant item : array) {
    ...
    const char* name = item["name"].is<const char*>()
      ? item["name"].as<const char*>()
      : (item["title"].is<const char*>() ? item["title"].as<const char*>() : "");
    const char* url = item["url"].is<const char*>()
      ? item["url"].as<const char*>()
      : (item["stream"].is<const char*>() ? item["stream"].as<const char*>() : "");
    appendPlaylistEntry(entries, String(name), String(url));
  }
  return !entries.empty();
}

bool writePlaylistCsvFile(const String& filename, const std::vector<ParsedPlaylistEntry>& entries, String* errorMessage) {
  File out = LittleFS.open(filename, "w", true);
  if (!out) {
    if (errorMessage) *errorMessage = "Failed to write converted playlist";
    return false;
  }

  for (size_t i = 0; i < entries.size(); ++i) {
    String safeName = sanitizeImportedPlaylistField(entries[i].name);
    String safeUrl = sanitizeImportedPlaylistField(entries[i].url);
...
bool importPlaylistFileToCsv(const String& sourcePath, String& targetCsvPath, String* errorMessage) {
  String source = normalizeFsPath(sourcePath);
  if (!LittleFS.exists(source)) {
    if (errorMessage) *errorMessage = "Uploaded playlist file not found";
    return false;
  }

  std::vector<ParsedPlaylistEntry> entries;
  if (source.endsWith(".json")) {
    File file = LittleFS.open(source, "r");
    if (!file) {
      if (errorMessage) *errorMessage = "Failed to open uploaded JSON";
      return false;
    }
    String content = file.readString();
    file.close();
    if (!parseJsonPlaylist(content, entries)) {
      if (errorMessage) *errorMessage = "JSON is not a playlist";
      return false;
    }
  } else {
    File file = LittleFS.open(source, "r");
    if (!file) {
      if (errorMessage) *errorMessage = "Failed to open uploaded playlist";
      return false;
    }

    bool parsed = false;
    if (source.endsWith(".m3u") || source.endsWith(".m3u8")) parsed = parseM3UPlaylist(file, entries);
    else if (source.endsWith(".pls")) parsed = parsePLSPlaylist(file, entries);
    else if (source.endsWith(".csv") || source.endsWith(".txt")) parsed = parseTextLikePlaylist(file, entries);
    file.close();

    if (!parsed) {
      if (errorMessage) *errorMessage = "No valid stations found in playlist";
      return false;
    }
  }

  if (entries.empty()) {
    if (errorMessage) *errorMessage = "Playlist is empty";
    return false;
  }

  targetCsvPath = source;
  const int dotPos = targetCsvPath.lastIndexOf('.');
  if (dotPos >= 0) targetCsvPath = targetCsvPath.substring(0, dotPos);
  targetCsvPath += ".csv";

  return writePlaylistCsvFile(targetCsvPath, entries, errorMessage);
}
...
bool isWebFileTransferActive() {
  return g_uploadInProgress || g_pendingTarImport || g_archiveImportInProgress;
}

void beginArchiveImportProtection() {
  if (g_archiveImportRuntimeStateValid) return;

  g_archiveImportRuntimeState.radioWasStarted = radioStarted;
  g_archiveImportRuntimeState.radioWasPaused = isPaused;
  g_archiveImportRuntimeStateValid = true;

  setDisplayTaskPaused(true);
  g_archiveImportInProgress = true;

  if (g_archiveImportRuntimeState.radioWasStarted) {
    audio.stopSong();
    radioStarted = false;
    isPaused = false;
    delay(120);
...
void endArchiveImportProtection() {
  if (g_archiveImportRuntimeStateValid && g_archiveImportRuntimeState.radioWasStarted && wifiConnected && streamCount > 0) {
    startRadio();
    if (g_archiveImportRuntimeState.radioWasPaused && radioStarted && !isPaused) {
      audio.pauseResume();
      isPaused = true;
    }
  }

  g_archiveImportRuntimeState = ArchiveImportRuntimeState{};
  g_archiveImportRuntimeStateValid = false;
  g_archiveImportInProgress = false;
...
static void handleSetVuBars() {
  ...
  if (server.hasArg("bottom")) vuBarsColorBottom = strtol(server.arg("bottom").c_str(), nullptr, 16);
  if (server.hasArg("top")) vuBarsColorTop = strtol(server.arg("top").c_str(), nullptr, 16);
  scheduleDisplaySettingsSave();
  redrawScreen();
  server.send(200, "text/plain", "OK");
}

static float argFloatSetting(const char* name, float current, float lo, float hi) {
  if (!server.hasArg(name)) return current;
  return constrain(server.arg(name).toFloat(), lo, hi);
}

static void handleSetFft() {
  const int maxW = gfx ? gfx->width() : 480;
  const int maxH = gfx ? gfx->height() : 320;
  if (server.hasArg("x")) fftWindowX = constrain(server.arg("x").toInt(), 0, maxW);
  if (server.hasArg("y")) fftWindowY = constrain(server.arg("y").toInt(), 0, maxH);
  if (server.hasArg("w")) fftWindowW = constrain(server.arg("w").toInt(), 40, maxW);
  if (server.hasArg("h")) fftWindowH = constrain(server.arg("h").toInt(), 40, maxH);
  if (server.hasArg("bands")) fftBands = constrain(server.arg("bands").toInt(), 1, FFT_BANDS);
  if (server.hasArg("segments")) fftSegments = constrain(server.arg("segments").toInt(), 1, 80);
  if (server.hasArg("minBin")) fftMinBin = constrain(server.arg("minBin").toInt(), 0, FFT_BANDS - 1);
  if (server.hasArg("maxBin")) fftMaxBin = constrain(server.arg("maxBin").toInt(), fftMinBin, FFT_BANDS - 1);
  if (server.hasArg("aggregation")) fftAggregation = constrain(server.arg("aggregation").toInt(), 0, 2);
  if (server.hasArg("curve")) fftCurve = constrain(server.arg("curve").toInt(), 0, 2);
  if (server.hasArg("profile")) {
    fftProfile = constrain(server.arg("profile").toInt(), 0, 2);
    audio.setSpectrumProfile(fftProfile);
  }
  fftGain = argFloatSetting("gain", fftGain, 0.0f, 8.0f);
  fftPreGain = argFloatSetting("preGain", fftPreGain, 0.0f, 8.0f);
  fftNoiseGate = argFloatSetting("noiseGate", fftNoiseGate, 0.0f, 120.0f);
  fftNoiseFloor = argFloatSetting("noiseFloor", fftNoiseFloor, 0.0f, 160.0f);
  fftSensitivity = argFloatSetting("sensitivity", fftSensitivity, 0.0f, 4.0f);
  fftGamma = argFloatSetting("gamma", fftGamma, 0.1f, 3.0f);
  fftLogStrength = argFloatSetting("logStrength", fftLogStrength, 0.1f, 12.0f);
  fftAttack = argFloatSetting("attack", fftAttack, 0.0f, 1.0f);
  fftRelease = argFloatSetting("release", fftRelease, 0.0f, 1.0f);
  fftLowBoost = argFloatSetting("lowBoost", fftLowBoost, 0.0f, 4.0f);
  fftMidBoost = argFloatSetting("midBoost", fftMidBoost, 0.0f, 4.0f);
  fftHighBoost = argFloatSetting("highBoost", fftHighBoost, 0.0f, 4.0f);
  if (server.hasArg("barGap")) fftBarGap = constrain(server.arg("barGap").toInt(), 0, 12);
  if (server.hasArg("sideMargin")) fftSideMargin = constrain(server.arg("sideMargin").toInt(), 0, 80);
  if (server.hasArg("topMargin")) fftTopMargin = constrain(server.arg("topMargin").toInt(), 0, 100);
  if (server.hasArg("bottomMargin")) fftBottomMargin = constrain(server.arg("bottomMargin").toInt(), 0, 100);
  if (server.hasArg("minLitSegments")) fftMinLitSegments = constrain(server.arg("minLitSegments").toInt(), 0, 10);
  if (server.hasArg("peakHold")) fftPeakHold = server.arg("peakHold").toInt() != 0;
  if (server.hasArg("peakFall")) fftPeakFall = constrain(server.arg("peakFall").toInt(), 0, 20);
  if (server.hasArg("mirror")) fftMirror = server.arg("mirror").toInt() != 0;
  if (server.hasArg("invert")) fftInvert = server.arg("invert").toInt() != 0;
  if (server.hasArg("bottom")) fftColorBottom = strtol(server.arg("bottom").c_str(), nullptr, 16);
  if (server.hasArg("top")) fftColorTop = strtol(server.arg("top").c_str(), nullptr, 16);
  if (server.hasArg("peak")) fftColorPeak = strtol(server.arg("peak").c_str(), nullptr, 16);
  if (server.hasArg("off")) fftColorOff = strtol(server.arg("off").c_str(), nullptr, 16);
  scheduleDisplaySettingsSave();
  refreshVuOnly();
  redrawScreen();
  server.send(200, "text/plain", "OK");
...
static void handleSetDisplay() {
  if (server.hasArg("invert")) displayInverted = server.arg("invert").toInt() != 0;
  if (gfx) gfx->invertDisplay(displayInverted);
  scheduleDisplaySettingsSave();
  redrawScreen();
  server.send(200, "text/plain", "OK");
}
...
static void handleSetQuote() {
  quoteMarqueeEnabled = false;
  server.send(200, "text/plain", "0");
}

static void handleSetScreensaver() {
  bool changed = false;
  if (server.hasArg("enabled")) {
    setScreensaverEnabled(server.arg("enabled").toInt() != 0);
    changed = true;
  }
  if (server.hasArg("idle")) {
    setScreensaverIdleSeconds(server.arg("idle").toInt());
    changed = true;
  }
  if (changed) {
    saveRuntimeSettings();
  ...
  String json = "{";
  json += "\"enabled\":";
  json += screensaverEnabled ? "true" : "false";
  json += ",\"idleSeconds\":" + String(screensaverIdleSeconds);
  json += ",\"file\":\"/screensaver.txt\"";
  json += "}";
...
#ifdef WS2812B_PIN
  json += ",\"colorOrder\":\"" + String(ledMusic.getColorOrderName()) + "\"";
...
static void handleAPIVuMeter() {
  ...
  json += ",\"colorBottom\":" + String(vuBarsColorBottom);
  json += ",\"colorTop\":" + String(vuBarsColorTop);
...
static void handleAPIFft() {
  String json = "{";
  json += "\"x\":" + String(fftWindowX);
  json += ",\"y\":" + String(fftWindowY);
  json += ",\"w\":" + String(fftWindowW);
  json += ",\"h\":" + String(fftWindowH);
  json += ",\"bands\":" + String(fftBands);
  json += ",\"segments\":" + String(fftSegments);
  json += ",\"minBin\":" + String(fftMinBin);
  json += ",\"maxBin\":" + String(fftMaxBin);
  json += ",\"aggregation\":" + String(fftAggregation);
  json += ",\"curve\":" + String(fftCurve);
  json += ",\"profile\":" + String(fftProfile);
  json += ",\"gain\":" + String(fftGain, 2);
  json += ",\"preGain\":" + String(fftPreGain, 2);
  json += ",\"noiseGate\":" + String(fftNoiseGate, 1);
  json += ",\"noiseFloor\":" + String(fftNoiseFloor, 1);
  json += ",\"sensitivity\":" + String(fftSensitivity, 2);
  json += ",\"gamma\":" + String(fftGamma, 2);
  json += ",\"logStrength\":" + String(fftLogStrength, 2);
  json += ",\"attack\":" + String(fftAttack, 2);
  json += ",\"release\":" + String(fftRelease, 2);
  json += ",\"lowBoost\":" + String(fftLowBoost, 2);
  json += ",\"midBoost\":" + String(fftMidBoost, 2);
  json += ",\"highBoost\":" + String(fftHighBoost, 2);
  json += ",\"barGap\":" + String(fftBarGap);
  json += ",\"sideMargin\":" + String(fftSideMargin);
  json += ",\"topMargin\":" + String(fftTopMargin);
  json += ",\"bottomMargin\":" + String(fftBottomMargin);
  json += ",\"minLitSegments\":" + String(fftMinLitSegments);
  json += ",\"peakHold\":" + String(fftPeakHold ? 1 : 0);
  json += ",\"peakFall\":" + String(fftPeakFall);
  json += ",\"mirror\":" + String(fftMirror ? 1 : 0);
  json += ",\"invert\":" + String(fftInvert ? 1 : 0);
  json += ",\"colorBottom\":" + String(fftColorBottom);
  json += ",\"colorTop\":" + String(fftColorTop);
  json += ",\"colorPeak\":" + String(fftColorPeak);
  json += ",\"colorOff\":" + String(fftColorOff);
  json += "}";
  server.send(200, "application/json", json);
}

static void handleAPIVisualSettings() {
  String json = "{";
  json += "\"needleLX\":" + String(needlePosLX) + ",";
  json += "\"needleLY\":" + String(needlePosLY) + ",";
  json += "\"needleRX\":" + String(needlePosRX) + ",";
  json += "\"needleRY\":" + String(needlePosRY) + ",";
  json += "\"needleMinAngle\":" + String(needleMinAngle) + ",";
  json += "\"needleMaxAngle\":" + String(needleMaxAngle) + ",";
  json += "\"needleUpSpeed\":" + String(needleUpSpeed) + ",";
  json += "\"needleDownSpeed\":" + String(needleDownSpeed) + ",";
  json += "\"needleThickness\":" + String(needleThickness) + ",";
  json += "\"needleMainLen\":" + String(needleLenMain) + ",";
  json += "\"needleRedLen\":" + String(needleLenRed) + ",";
  json += "\"needleCenterY\":" + String(needleCY) + ",";
  json += "\"needleSpriteW\":" + String(needleSpriteWidth) + ",";
  json += "\"needleSpriteH\":" + String(needleSpriteHeight) + ",";
  json += "\"debugBorders\":" + String(debugBordersEnabled ? "true" : "false") + ",";
  json += "\"leftMirrored\":" + String(leftNeedleMirrored ? "true" : "false") + ",";
  json += "\"needleRotation\":" + String(needleRotation) + ",";
  json += "\"stationX\":" + String(stationNameX) + ",";
  json += "\"stationY\":" + String(stationNameY) + ",";
  json += "\"stationW\":" + String(stationScrollWidth) + ",";
  json += "\"title1X\":" + String(streamTitle1X) + ",";
  json += "\"title1Y\":" + String(streamTitle1Y) + ",";
  json += "\"title1W\":" + String(title1ScrollWidth) + ",";
  json += "\"title2X\":" + String(streamTitle2X) + ",";
  json += "\"title2Y\":" + String(streamTitle2Y) + ",";
  json += "\"title2W\":" + String(title2ScrollWidth) + ",";
  json += "\"bitrateX\":" + String(bitrateX) + ",";
  json += "\"bitrateY\":" + String(bitrateY) + ",";
  json += "\"clockX\":" + String(clockX) + ",";
  json += "\"clockY\":" + String(clockY) + ",";
  json += "\"volumeX\":" + String(volumeX) + ",";
  json += "\"volumeY\":" + String(volumeY) + ",";
  json += "\"ipX\":" + String(ipX) + ",";
  json += "\"ipY\":" + String(ipY) + ",";
  json += "\"quoteX\":" + String(quoteX) + ",";
  json += "\"quoteY\":" + String(quoteY) + ",";
  json += "\"quoteW\":" + String(quoteW) + ",";
  json += "\"marqueePauseMs\":" + String(marqueePauseMs);
  json += ",\"displayInverted\":" + String(displayInverted ? 1 : 0);
  json += "}";
  server.send(200, "application/json", json);
}
...
static void handleAPIScreensaver() {
  String json = "{";
  json += "\"enabled\":";
  json += screensaverEnabled ? "true" : "false";
  json += ",\"idleSeconds\":" + String(screensaverIdleSeconds);
  json += ",\"file\":\"/screensaver.txt\"";
  json += ",\"hasFile\":";
  json += LittleFS.exists("/screensaver.txt") ? "true" : "false";
  json += "}";
  server.send(200, "application/json", json);
}
...
static void handleSetLedConfig() {
  ...
  if (server.hasArg("order")) {
    String requestedOrder = server.arg("order");
    requestedOrder.toUpperCase();
    if (ledMusic.setColorOrder(requestedOrder.c_str())) {
...
static void handleSetVolume() {
  if (server.hasArg("v")) {
    currentVolume = constrain(server.arg("v").toInt(), kVolumePercentMin, kVolumePercentMax);
    applyCurrentVolume();
    scheduleVolumeSave();
...
static void handleSetTextPos() {
  if (server.hasArg("stationX")) stationNameX = nonNegativeCoord(server.arg("stationX").toInt());
  if (server.hasArg("stationY")) stationNameY = nonNegativeCoord(server.arg("stationY").toInt());
  if (server.hasArg("title1X")) streamTitle1X = nonNegativeCoord(server.arg("title1X").toInt());
  if (server.hasArg("title1Y")) streamTitle1Y = nonNegativeCoord(server.arg("title1Y").toInt());
  if (server.hasArg("title2X")) streamTitle2X = nonNegativeCoord(server.arg("title2X").toInt());
  if (server.hasArg("title2Y")) streamTitle2Y = nonNegativeCoord(server.arg("title2Y").toInt());
  ...
  if (server.hasArg("bitrateX")) bitrateX = nonNegativeCoord(server.arg("bitrateX").toInt());
  if (server.hasArg("bitrateY")) bitrateY = nonNegativeCoord(server.arg("bitrateY").toInt());
  if (server.hasArg("clockX")) { clockX = nonNegativeCoord(server.arg("clockX").toInt()); clockSpriteX = clockX; redrawClock = true; }
  if (server.hasArg("clockY")) { clockY = nonNegativeCoord(server.arg("clockY").toInt()); clockSpriteY = clockY; redrawClock = true; }
  if (server.hasArg("volumeX")) volumeX = nonNegativeCoord(server.arg("volumeX").toInt());
  if (server.hasArg("volumeY")) volumeY = nonNegativeCoord(server.arg("volumeY").toInt());
  if (server.hasArg("ipX")) ipX = nonNegativeCoord(server.arg("ipX").toInt());
  if (server.hasArg("ipY")) ipY = nonNegativeCoord(server.arg("ipY").toInt());
  if (server.hasArg("quoteX")) quoteX = nonNegativeCoord(server.arg("quoteX").toInt());
  if (server.hasArg("quoteY")) quoteY = nonNegativeCoord(server.arg("quoteY").toInt());
...
static void handleSetScreenEnabled() {
  if (!server.hasArg("screen") || !server.hasArg("enabled")) {
    server.send(400, "text/plain", "Missing screen or enabled parameter");
    return;
  }
  ...
  const ScreenDefinition* defs = getAllScreenDefinitions(count);
  for (size_t i = 0; i < count; ++i) {
    if (key != defs[i].key) continue;
    if (!defs[i].implemented) {
      server.send(400, "text/plain", "Screen is not implemented yet");
      return;
    }
    if (!setScreenEnabled(defs[i].id, enabled)) {
      server.send(409, "text/plain", "At least one screen must stay enabled");
      return;
    }
    if (!enabled && defs[i].id == getCurrentScreen()) {
      saveCurrentScreenSettings();
      if (ensureCurrentScreenEnabled()) {
        loadCurrentScreenSettings();
        loadBackground();
        redrawScreen();
      }
...
static void handleAPIScreens() {
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    json += "\"enabled\":" + String(isScreenEnabled(defs[i].id) ? "true" : "false");
    json += "}";
    first = false;
  }
  json += "]";
...
static void handleAPIUploadStatus() {
  String json = "{";
  json += "\"uploading\":";
  json += g_uploadInProgress ? "true" : "false";
  json += ",\"pendingImport\":";
  json += g_pendingTarImport ? "true" : "false";
  json += ",\"importing\":";
  json += g_archiveImportInProgress ? "true" : "false";
  json += ",\"busy\":";
  json += isWebFileTransferActive() ? "true" : "false";
  json += "}";
  server.send(200, "application/json", json);
}
...
static void handleUploadProcess() {
  ...
  if (upload.status == UPLOAD_FILE_START) {
    ...
    if (!filename.endsWith(".jpg") && !filename.endsWith(".jpeg") && !filename.endsWith(".png")
        ...
        && !filename.endsWith(".pls") && !filename.endsWith(".txt")) {
    ...
    if (uploadFile) {
      ...
      if (!g_uploadHadError) {
        if (isImageAssetFilename(filename)) {
          ...
          String importedPlaylistPath;
          String errorMessage;
          if (importPlaylistFileToCsv(filename, importedPlaylistPath, &errorMessage)) {
            if (importedPlaylistPath != filename) LittleFS.remove(filename);
            loadPlaylistMetadata(importedPlaylistPath);
            if (streamCount > 0) selectStream(0);
            g_uploadResponseMessage = "Playlist imported as " + importedPlaylistPath;
          } else if (!loadDisplayConfigFromFile(filename, &errorMessage)) {
            g_uploadHadError = true;
            g_uploadResponseMessage = "Display config import failed: " + errorMessage;
            Serial.printf("Display config import skipped: %s\n", errorMessage.c_str());
          }
        } else if (filename == "/screensaver.txt") {
          noteUserActivity();
          if (screensaverEnabled) setScreensaverEnabled(true);
          g_uploadResponseMessage = "Screensaver words uploaded";
        } else if (filename.endsWith(".csv") || filename.endsWith(".m3u") || filename.endsWith(".m3u8")
                   || filename.endsWith(".pls") || filename.endsWith(".txt")) {
          String importedPlaylistPath;
          String errorMessage;
          if (!importPlaylistFileToCsv(filename, importedPlaylistPath, &errorMessage)) {
            g_uploadHadError = true;
            g_uploadResponseMessage = "Playlist import failed: " + errorMessage;
          } else {
            if (importedPlaylistPath != filename) LittleFS.remove(filename);
            loadPlaylistMetadata(importedPlaylistPath);
            if (streamCount > 0) selectStream(0);
            g_uploadResponseMessage = importedPlaylistPath == filename
              ? "Playlist uploaded"
              : "Playlist converted to " + importedPlaylistPath;
          }
    ...
    if (uploadFilename.endsWith(".tar")) endArchiveImportProtection();
    totalWritten = 0;
    uploadFilename = "";
    g_uploadInProgress = false;
    g_uploadHadError = true;
...
static void handleAPIPlaylists() {
  ...
  while (file) {
    ...
    if (name.endsWith(".csv")) {
      if (!first) json += ",";
      json += "{\"name\":\"" + jsonEscape(name) + "\",\"selected\":" + String(name == currentPlaylistFile ? "true" : "false") + "}";
      first = false;
    }
    file = root.openNextFile();
  }
...
static void handleAPIBackgrounds() {
  ...
  while (file) {
    ...
    if (name.endsWith(".jpg") || name.endsWith(".jpeg")) {
      if (!first) json += ",";
      json += "{\"name\":\"" + jsonEscape(name) + "\",\"selected\":" + String(name == currentBackground ? "true" : "false") + "}";
      first = false;
    }
    file = root.openNextFile();
  }
...
static void handleViewPlaylist() {
  ...
  while (f.available()) {
    ...
    if (!first) json += ",";
    json += "{\"name\":\"" + jsonEscape(line.substring(0, tab1)) + "\",\"url\":\"" + jsonEscape(line.substring(tab1 + 1, tab2)) + "\"}";
    first = false;
  }
  f.close();
  json += "]";
...
static void handleAPIAudioInfo() {
  String json = "{";
  json += "\"stationName\":\"" + jsonEscape(currentStreamName) + "\",";
  json += "\"playlistStation\":\"" + jsonEscape(currentStreamName) + "\",";
  json += "\"stationNumber\":" + String(currentStreamIndex + 1) + ",";
  json += "\"stationCount\":" + String(streamCount) + ",";
  json += "\"icyStation\":\"" + jsonEscape(icy_station.length() ? icy_station : currentStreamName) + "\",";
  json += "\"icyTitle\":\"" + jsonEscape(icy_streamtitle.length() ? icy_streamtitle : "---") + "\",";
  json += "\"icyBitrate\":\"" + jsonEscape(icy_bitrate.length() ? icy_bitrate : String(audio.getBitRate() / 1000)) + "\",";
  json += "\"codec\":\"" + jsonEscape(audio_codec.length() ? audio_codec : "unknown") + "\",";
  json += "\"frequency\":\"" + jsonEscape(audio_frequency.length() ? audio_frequency : "---") + "\",";
  json += "\"bitness\":\"" + jsonEscape(audio_bitness.length() ? audio_bitness : "---") + "\",";
...
static uint32_t parseHexOrDec(const String& value) {
  String text = value;
  text.trim();
  if (text.startsWith("0x") || text.startsWith("0X")) {
    return strtoul(text.c_str() + 2, nullptr, 16);
  }
  return strtoul(text.c_str(), nullptr, 0);
}

static void handleSetRemote() {
  bool restartReceiver = false;

  if (server.hasArg("enabled")) {
    restartReceiver |= irRemoteSetEnabled(server.arg("enabled").toInt() != 0);
  }
  if (server.hasArg("pin")) {
    restartReceiver |= irRemoteSetPin(constrain(server.arg("pin").toInt(), 0, 255));
  }
  if (server.hasArg("repeatDelayMs")) {
    irRemoteSetRepeatDelay(constrain(server.arg("repeatDelayMs").toInt(), 0, 2000));
  }
  if (server.hasArg("action") && server.hasArg("command")) {
    IrRemoteAction action;
    if (!irRemoteActionFromKey(server.arg("action"), action)) {
      server.send(400, "text/plain", "Unknown remote action");
      return;
    }
    irRemoteSetCommand(action, parseHexOrDec(server.arg("command")));
  }

  String errorMessage;
  if (!irRemoteSaveConfig(&errorMessage)) {
    server.send(500, "text/plain", errorMessage);
    return;
  }

  if (restartReceiver) irRemoteBegin();
  server.send(200, "application/json", irRemoteConfigJson());
}

static void handleAPIRemote() {
  server.send(200, "application/json", irRemoteConfigJson());
}

static void handleAPIRemoteLast() {
  server.send(200, "application/json", irRemoteLastCodeJson());
}

static String getWifiKey(const char* prefix, int index) {
  return String(prefix) + String(index + 1);
}
...
static void handleAPIWifi() {
  ...
  for (int i = 0; i < 3; ++i) {
    String ssid = wifiPrefs.getString(getWifiKey("wifi_ssid", i).c_str(), "");
    String pass = wifiPrefs.getString(getWifiKey("wifi_pass", i).c_str(), "");
    if (i == 0 && ssid.length() == 0) {
      ssid = wifiPrefs.getString("wifi_ssid", "");
      pass = wifiPrefs.getString("wifi_pass", "");
    }
    if (i > 0) json += ",";
    json += "{\"ssid\":\"" + jsonEscape(ssid) + "\",\"pass\":\"" + jsonEscape(pass) + "\"}";
  }
  wifiPrefs.end();
  json += "]}";
  server.send(200, "application/json", json);
...
static void handleSetWifiList() {
  Preferences wifiPrefs;
  wifiPrefs.begin("radio", false);
  for (int i = 0; i < 3; ++i) {
    String ssidArg = String("ssid") + String(i);
    String passArg = String("pass") + String(i);
    String ssid = server.hasArg(ssidArg) ? server.arg(ssidArg) : "";
    String pass = server.hasArg(passArg) ? server.arg(passArg) : "";
    ssid.trim();
    pass.trim();
    wifiPrefs.putString(getWifiKey("wifi_ssid", i).c_str(), ssid);
    wifiPrefs.putString(getWifiKey("wifi_pass", i).c_str(), pass);
    if (i == 0) {
      wifiPrefs.putString("wifi_ssid", ssid);
      wifiPrefs.putString("wifi_pass", pass);
    }
  }
  wifiPrefs.end();
  server.send(200, "text/plain", "OK");
...
void initWebServer() {
  ...
  server.on("/api/visualsettings", handleAPIVisualSettings);
  ...
  server.on("/api/screensaver", handleAPIScreensaver);
  ...
  server.on("/api/uploadstatus", handleAPIUploadStatus);
  ...
  server.on("/setremote", handleSetRemote);
  server.on("/api/remote", handleAPIRemote);
  server.on("/api/remotelast", handleAPIRemoteLast);
  server.on("/api/wifi", handleAPIWifi);
  server.on("/setwifilist", handleSetWifiList);
