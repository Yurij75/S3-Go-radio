...
static void rebootAfterOTA(void*) {
  delay(1200);
  ESP.restart();
  vTaskDelete(nullptr);
}
...
void handleOTAUpdateResult() {
  const bool success = !g_otaRejected && !Update.hasError();
  ...
  if (success) {
    xTaskCreate(rebootAfterOTA, "ota_reboot", 2048, nullptr, 1, nullptr);
  }
}
...
void handleOTAUpload() {
  ...
  if (upload.status == UPLOAD_FILE_START) {
    ...
    if (!filename.endsWith(".bin")) {
      g_otaRejected = true;
      setOTAError("OTA failed: choose a firmware .bin file.");
      return;
    }
    ...
    if (g_otaRejected || Update.hasError()) {
      return;
    }

    if (upload.totalSize == 0 && upload.currentSize > 0 && upload.buf[0] != ESP_IMAGE_HEADER_MAGIC) {
      g_otaRejected = true;
      Update.abort();
      setOTAError("OTA failed: this file is not an ESP32 firmware image.");
      return;
    }
