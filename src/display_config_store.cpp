...
extern uint16_t fftColorBottom, fftColorTop, fftColorPeak, fftColorOff;
...
constexpr int kVuBarsMaxBandCount = 32;
...
struct VisualSnapshot {
  ...
  float fftNoiseGate;
  float fftNoiseFloor;
  float fftSensitivity;
  float fftGamma;
  float fftLogStrength;
  float fftAttack;
  float fftRelease;
  float fftLowBoost;
  float fftMidBoost;
  ...
  int fftTopMargin;
  int fftBottomMargin;
  int fftMinLitSegments;
  bool fftPeakHold;
  int fftPeakFall;
  bool fftMirror;
  bool fftInvert;
  uint16_t fftColorBottom;
  uint16_t fftColorTop;
  uint16_t fftColorPeak;
  uint16_t fftColorOff;
  int needleMinAngle;
  int needleMaxAngle;
  int needleUpSpeed;
  int needleDownSpeed;
...
VisualSnapshot takeVisualSnapshot() {
  ...
  // snapshot.vuBarsColorBottom = vuBarsColorBottom;
  // snapshot.vuBarsColorTop = vuBarsColorTop;
...
void restoreVisualSnapshot(const VisualSnapshot& snapshot) {
  ...
  // vuBarsColorBottom = snapshot.vuBarsColorBottom;
  // vuBarsColorTop = snapshot.vuBarsColorTop;
...
void serializeCurrentVisualState(JsonObject screenObj) {
  ...
  // vu["barsColorBottom"] = vuBarsColorBottom;
  // vu["barsColorTop"] = vuBarsColorTop;
...
void applyCurrentVisualState(JsonObjectConst screenObj) {
  ...
  if (!text.isNull()) {
    ...
    stationNameX = nonNegativeCoord(text["stationX"] | stationNameX);
    stationNameY = nonNegativeCoord(text["stationY"] | stationNameY);
    stationScrollWidth = text["stationW"] | stationScrollWidth;
    streamTitle1X = nonNegativeCoord(text["title1X"] | streamTitle1X);
    streamTitle1Y = nonNegativeCoord(text["title1Y"] | streamTitle1Y);
    title1ScrollWidth = text["title1W"] | title1ScrollWidth;
    streamTitle2X = nonNegativeCoord(text["title2X"] | streamTitle2X);
    streamTitle2Y = nonNegativeCoord(text["title2Y"] | streamTitle2Y);
    title2ScrollWidth = text["title2W"] | title2ScrollWidth;
    bitrateX = nonNegativeCoord(text["bitrateX"] | bitrateX);
    bitrateY = nonNegativeCoord(text["bitrateY"] | bitrateY);
    clockX = nonNegativeCoord(text["clockX"] | clockX);
    clockY = nonNegativeCoord(text["clockY"] | clockY);
    clockSpriteX = clockX;
    clockSpriteY = clockY;
    volumeX = nonNegativeCoord(text["volumeX"] | volumeX);
    volumeY = nonNegativeCoord(text["volumeY"] | volumeY);
    ipX = nonNegativeCoord(text["ipX"] | ipX);
    ipY = nonNegativeCoord(text["ipY"] | ipY);
  ...
  if (!vu.isNull()) {
    ...
    // vuBarsColorBottom = vu["barsColorBottom"] | vuBarsColorBottom;
    // vuBarsColorTop = vu["barsColorTop"] | vuBarsColorTop;
...
bool saveDisplayConfigToFile(const String& filename, String* errorMessage) {
  ...
  doc["activeScreen"] = "vumeter2"; // не трогать не менять !!!!
...
String buildCurrentProfileHeader() {
  ...
  header += "#define DISPLAY_PROFILE_STATION_X " + String(nonNegativeCoord(stationNameX)) + "\n";
  header += "#define DISPLAY_PROFILE_STATION_Y " + String(nonNegativeCoord(stationNameY)) + "\n";
  header += "#define DISPLAY_PROFILE_TITLE1_X " + String(nonNegativeCoord(streamTitle1X)) + "\n";
  header += "#define DISPLAY_PROFILE_TITLE1_Y " + String(nonNegativeCoord(streamTitle1Y)) + "\n";
  header += "#define DISPLAY_PROFILE_TITLE2_X " + String(nonNegativeCoord(streamTitle2X)) + "\n";
  header += "#define DISPLAY_PROFILE_TITLE2_Y " + String(nonNegativeCoord(streamTitle2Y)) + "\n";
  header += "#define DISPLAY_PROFILE_BITRATE_X " + String(nonNegativeCoord(bitrateX)) + "\n";
  header += "#define DISPLAY_PROFILE_BITRATE_Y " + String(nonNegativeCoord(bitrateY)) + "\n";
  header += "#define DISPLAY_PROFILE_IP_X " + String(nonNegativeCoord(ipX)) + "\n";
  header += "#define DISPLAY_PROFILE_IP_Y " + String(nonNegativeCoord(ipY)) + "\n";
  header += "#define DISPLAY_PROFILE_CLOCK_X " + String(nonNegativeCoord(clockX)) + "\n";
  header += "#define DISPLAY_PROFILE_CLOCK_Y " + String(nonNegativeCoord(clockY)) + "\n";
  header += "#define DISPLAY_PROFILE_VOLUME_X " + String(nonNegativeCoord(volumeX)) + "\n";
  header += "#define DISPLAY_PROFILE_VOLUME_Y " + String(nonNegativeCoord(volumeY)) + "\n\n";
