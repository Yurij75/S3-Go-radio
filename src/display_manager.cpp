...
constexpr int kMarqueeStepPixels = 2;
...
FontLineMetrics getFontLineMetrics(const GFXfont* font) {
  ...
  static FontLineMetrics cachedMetrics;
  if (font == cachedFont) {
...
void buildMetadataLines(String (&lines)[3]) {
  ...
  if (primaryMarqueeEnabled && !secondaryMarqueeEnabled) {
    String combined = lines[0];
    if (lines[1].length() && lines[1] != "---") {
      combined += " - " + lines[1];
    }
    if (lines[2].length() && lines[2] != "---") {
    ...
    lines[0] = combined.length() ? combined : "---";
  }
}
...
void advanceMarqueeState(MarqueeLineState& state) {
  if (state.pauseTicks > 0) {
    state.pauseTicks--;
    return;
  }

  state.offset += kMarqueeStepPixels;
  if (state.offset >= state.cycleWidth) {
    state.offset = 0;
    state.pauseTicks = marqueePauseTicks();
  }
}
...
void drawMarqueeLine(int index, const String& text, MarqueeLineState& state, int x, int baselineY, uint16_t color, const GFXfont* font) {
  ...
  // Не пересчитываем повторно — caller уже посчитал это же значение в state.cycleWidth
  const int repeatedAdvance = state.cycleWidth;
...
void drawMetadataMarquees(bool advance) {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    if (cycle <= clipWidth) {
      ...
      advanceMarqueeState(g_marqueeLines[i]);
    }
...
static void drawFlacBitrateLine() {
  gfx->setFont(&BITRATE_FONT);
  gfx->setTextColor(colorBitrate);
  gfx->setCursor(bitrateX, bitrateY);
  ...
  int br = icy_bitrate.toInt();
  if (br <= 0) br = audio.getBitRate() / 1000;
  String bitrateText = br > 0 ? String(br) + "k" : "---";
  bitrateText = trimTextToWidth(gfx, bitrateText, 300);
  gfx->print(bitrateText);
}
...
static bool shouldShowVu(ScreenId screen) {
  return vuMeterEnabled && (screen == ScreenId::VuMeter2 || screen == ScreenId::VuMeter3);
}

static bool shouldShowFft(ScreenId screen) {
  return screen == ScreenId::Fft;
}
...
static bool shouldShowFlacInfo(ScreenId screen) {
  return screen == ScreenId::FlacInfo;
}

static bool shouldShowPrimaryMarquee(ScreenId screen) {
  return primaryMarqueeEnabled && isScreenImplemented(screen) && screen != ScreenId::FlacInfo;
}

static bool shouldShowSecondaryMarquee(ScreenId screen) {
  return secondaryMarqueeEnabled && isScreenImplemented(screen) && screen != ScreenId::FlacInfo;
}
...
static void renderScreenToCanvas(Arduino_Canvas* target, ScreenId currentScreen, bool resetMarquees, bool forceClockRedraw) {
  ...
  if (shouldShowMetadata(currentScreen) || shouldShowFlacInfo(currentScreen)) {
    drawDataLines();
  } else {
  ...
  if (shouldShowClock(currentScreen)) {
    // SCREEN ROUTE: Clock1 / Clock2 -> drawClockHMSS()
    drawClockHMSS();
  }
  if (shouldShowVu(currentScreen) && isVuNeedlesScreen(currentScreen)) {
    // SCREEN ROUTE: VuMeter2 -> drawVuNeedleChannel() for L/R needles
    ...
    // SCREEN ROUTE: VuMeter3 -> drawVuCardioWindowDynamic()
    drawVuCardioWindowDynamic(false);
  }
  if (shouldShowFft(currentScreen)) {
    // SCREEN ROUTE: Fft -> drawFftWindow()
    drawFftWindow();
  }
  ...
  while (true) {
    ...
    if (xSemaphoreTake(xMutex, pdMS_TO_TICKS(5)) == pdTRUE) {
      if (bgSprite != nullptr && needleSpriteL != nullptr && needleSpriteR != nullptr) {
        ...
        } else if (shouldShowFft(currentScreen)) {
          drawFftWindow();
    ...
    if ((shouldShowMetadata(currentScreen) || shouldShowPrimaryMarquee(currentScreen) || shouldShowSecondaryMarquee(currentScreen)) &&
    ...
    vTaskDelay(pdMS_TO_TICKS(shouldShowFft(currentScreen) ? 5 : DISPLAY_TASK_DELAY));
  }
}
...
void redrawScreenLocked() {
  ...
  if (shouldShowMetadata(currentScreen)) {
    drawVolumeDisplay();
    g_lastMetadataFrameMs = now;
  } else if (shouldShowPrimaryMarquee(currentScreen) || shouldShowSecondaryMarquee(currentScreen)) {
    g_lastMetadataFrameMs = now;
  }
}
...
void refreshVuOnly() {
  const ScreenId currentScreen = getCurrentScreen();
  if (!shouldShowVu(currentScreen) && !shouldShowFft(currentScreen)) return;
