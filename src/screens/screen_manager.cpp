...
ScreenState g_screenSequence[] = {
  ...
  {ScreenId::Fft, true, kDefaultScreenDurationMs},
  {ScreenId::StationInfo, true, kDefaultScreenDurationMs},
  {ScreenId::FlacInfo, true, kDefaultScreenDurationMs},
};
...
bool setScreenEnabled(ScreenId id, bool enabled) {
  const int index = findScreenIndex(id);
  if (index < 0) return false;
  if (!enabled && g_screenSequence[index].enabled && getEnabledScreenCount() <= 1) {
    return false;
  }
...
size_t getEnabledScreenCount() {
  size_t enabledCount = 0;
  for (size_t i = 0; i < screenCount(); ++i) {
    if (g_screenSequence[i].enabled) ++enabledCount;
  }
  return enabledCount;
}

bool ensureCurrentScreenEnabled() {
  if (g_screenSequence[g_currentScreenIndex].enabled) return true;
  if (getEnabledScreenCount() == 0) {
    g_screenSequence[g_currentScreenIndex].enabled = true;
    return true;
  }
