...
class ps_ptr {
  ...
  public:
    ...
    /**
     * @brief Allocate memory suitable for DMA access (MALLOC_CAP_DMA).
     */
    bool alloc_dma(std::size_t size, const char* alloc_name = nullptr) {
        size = (size + 15) & ~15; // Align to 16 bytes
        reset();
        if (alloc_name) { set_name(alloc_name); }
        void* raw_mem = heap_caps_malloc(size, MALLOC_CAP_DMA);
        if (!raw_mem) {
            printf("OOM: failed to allocate %zu bytes (DMA) for %s\n", size, name ? name : "unnamed");
            allocated_size = 0;
            return false;
        }
    ...
    bool alloc_array_dma(std::size_t count, const char* alloc_name = nullptr) {
        if (alloc_name) { set_name(alloc_name); }
        return alloc_dma(sizeof(T) * count, alloc_name);
    }
    // —————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
