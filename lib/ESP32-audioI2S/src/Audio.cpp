...
constexpr size_t AUDIO_STACK_SIZE = 14000;
...
bool Audio::setPinout(uint8_t BCLK, uint8_t LRC, uint8_t DOUT, int8_t MCLK) {
    ...
    // Allocate FFT buffers in DMA-capable memory to allow DMA usage / safe cache handling
    m_fft_items.buffer.alloc_array_dma(m_fft_items.SIZE, "buffer");
    m_fft_items.window.alloc_array_dma(m_fft_items.SIZE, "window");
    m_fft_items.work.alloc_array_dma(m_fft_items.SIZE * 2, "work");
...
uint8_t Audio::getSpectrum(uint8_t* bands, uint8_t maxBands) {
    if (!bands || maxBands == 0 || !m_f_running) return 0;

    const uint8_t count = min<uint8_t>(maxBands, m_fft_items.BANDS);
    for (uint8_t i = 0; i < count; ++i) {
        bands[i] = m_fft_items.spectrum[i];
    }
    return count;
}
void Audio::setSpectrumProfile(uint8_t profile) {
    profile = min<uint8_t>(profile, 2);
    if (m_fft_items.profile == profile) return;
    m_fft_items.profile = profile;
    resetSpectrumAnalyzer();
}
// —————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
void Audio::resetSpectrumAnalyzer() {
    if (m_fft_items.buffer.valid()) {
        std::fill(m_fft_items.buffer.get(), m_fft_items.buffer.get() + m_fft_items.SIZE, 0.0f);
    }
    if (m_fft_items.work.valid()) {
        std::fill(m_fft_items.work.get(), m_fft_items.work.get() + m_fft_items.SIZE * 2, 0.0f);
    }
    std::fill(std::begin(m_fft_items.spec_smooth), std::end(m_fft_items.spec_smooth), 0.0f);
    std::fill(std::begin(m_fft_items.spectrum), std::end(m_fft_items.spectrum), 0);
    m_fft_items.buffer_index = 0;
    m_fft_items.pos = 0;
    m_fft_items.gain = 1.0f;
    m_fft_items.last_ms = millis();
}
...
void Audio::processSpectrum() {
    if (!m_fft_items.initialized || !m_fft_items.buffer.valid() || !m_fft_items.work.valid()) return;

    uint32_t now = millis();
    if (now - m_fft_items.last_ms < 50) return;
    m_fft_items.last_ms = now;

    // ========== 1. Оконное сглаживание ==========
    const uint16_t oldest = m_fft_items.buffer_index;
    for (int i = 0; i < m_fft_items.SIZE; i++) {
        const uint16_t src = (oldest + i) % m_fft_items.SIZE;
        m_fft_items.work[2 * i] = m_fft_items.buffer[src] * m_fft_items.window[i];
    ...
    // ========== 3. Магнитуды (линейные) для каждой частотной корзины ==========
    float magnitude[FFT_SIZE / 2] = {0};
    ...
    // ========== 4. Мел-шкала для 32 полос ==========
    static constexpr float mel_edges[FFT_BANDS + 1] = {
        20.0f, 40.0f, 80.0f, 120.0f, 200.0f, 300.0f, 430.0f, 530.0f,
        640.0f, 760.0f, 890.0f, 1030.0f, 1180.0f, 1340.0f, 1510.0f, 1690.0f,
        1880.0f, 2080.0f, 2290.0f, 2510.0f, 2740.0f, 2980.0f, 3230.0f, 3490.0f,
        4000.0f, 4500.0f, 5000.0f, 6000.0f, 7000.0f, 8000.0f, 10000.0f,
        15000.0f, 20000.0f
    };
    //     40.0f, 80.0f, 130.0f, 190.0f, 260.0f, 340.0f,      // бас (6 полос)
    //     430.0f, 530.0f, 640.0f, 760.0f, 890.0f, 1030.0f,    // нижняя середина (6)
    //     1180.0f, 1340.0f, 1510.0f, 1690.0f, 1880.0f,        // середина (5)
    //     2080.0f, 2290.0f, 2510.0f, 2740.0f, 2980.0f,        // середина (5)
    //     3230.0f, 3490.0f, 3760.0f, 4040.0f, 4330.0f,        // высокие (5)
    //     4630.0f, 4940.0f, 5260.0f, 5590.0f, 20000.0f        // высокие (5) — итого 32
    // };

    static constexpr float analyzer_edges[FFT_BANDS + 1] = {
        20.0f, 40.0f, 80.0f, 120.0f, 200.0f, 300.0f, 430.0f, 530.0f,
        640.0f, 760.0f, 890.0f, 1030.0f, 1180.0f, 1340.0f, 1510.0f, 1690.0f,
        1880.0f, 2080.0f, 2290.0f, 2510.0f, 2740.0f, 2980.0f, 3230.0f, 3490.0f,
        4000.0f, 4500.0f, 5000.0f, 6000.0f, 7000.0f, 8000.0f, 10000.0f,
    ...
    //     40.0f, 60.0f, 90.0f, 130.0f, 180.0f, 220.0f, 260.0f, 310.0f,
    //     350.0f, 390.0f, 440.0f, 480.0f, 525.0f, 650.0f, 825.0f, 1000.0f,
    //     1300.0f, 1600.0f, 2050.0f, 2500.0f, 3000.0f, 4000.0f, 5125.0f, 6250.0f,
    //     8000.0f, 10000.0f, 12000.0f, 13000.0f, 14000.0f, 15000.0f, 16000.0f,
    //     17000.0f, 18000.0f
    // };

    static constexpr float classic_edges[FFT_BANDS + 1] = {
        20.0f, 40.0f, 80.0f, 120.0f, 200.0f, 300.0f, 430.0f, 530.0f,
        640.0f, 760.0f, 890.0f, 1030.0f, 1180.0f, 1340.0f, 1510.0f, 1690.0f,
        1880.0f, 2080.0f, 2290.0f, 2510.0f, 2740.0f, 2980.0f, 3230.0f, 3490.0f,
        4000.0f, 4500.0f, 5000.0f, 6000.0f, 7000.0f, 8000.0f, 10000.0f,
    ...
    //     // Бас — 10 полос
    //     20.0f, 32.0f, 44.0f, 56.0f, 70.0f, 85.0f, 100.0f, 120.0f, 140.0f, 160.0f, 180.0f,
    //     // Нижняя середина — 6 полос
    //     210.0f, 240.0f, 270.0f, 300.0f, 340.0f, 380.0f,
    //     // Середина — 6 полос
    //     420.0f, 460.0f, 520.0f, 560.0f, 610.0f, 660.0f,
    //     // Верхняя середина — 5 полос
    //     720.0f, 770.0f, 820.0f, 880.0f, 946.0f,
    //     // Высокие — 5 полос (до 20 кГц)
    //     1300.0f, 3500.0f, 8000.0f, 13000.0f, 20000.0f
    // };

    static constexpr float smooth_mel_edges[FFT_BANDS + 1] = {
        20.0f, 40.0f, 80.0f, 120.0f, 200.0f, 300.0f, 430.0f, 530.0f,
        640.0f, 760.0f, 890.0f, 1030.0f, 1180.0f, 1340.0f, 1510.0f, 1690.0f,
        1880.0f, 2080.0f, 2290.0f, 2510.0f, 2740.0f, 2980.0f, 3230.0f, 3490.0f,
        4000.0f, 4500.0f, 5000.0f, 6000.0f, 7000.0f, 8000.0f, 10000.0f,
    ...
    //     40.0f, 80.0f, 130.0f, 190.0f, 260.0f, 340.0f, 430.0f, 530.0f,
    //     640.0f, 760.0f, 890.0f, 1030.0f, 1180.0f, 1340.0f, 1510.0f, 1690.0f,
    //     1880.0f, 2080.0f, 2290.0f, 2510.0f, 2740.0f, 2980.0f, 3230.0f, 3490.0f,
    //     3760.0f, 4040.0f, 4330.0f, 4630.0f, 4940.0f, 5260.0f, 5590.0f,
    //     9000.0f, 18000.0f
    // };
    const uint8_t spectrum_profile = min<uint8_t>(m_fft_items.profile, 2);
    const float* band_edges = spectrum_profile == 1 ? classic_edges : (spectrum_profile == 2 ? smooth_mel_edges : analyzer_edges);

    float band_energy[FFT_BANDS] = {0};
    float band_sum[FFT_BANDS] = {0};
    float band_peak[FFT_BANDS] = {0};
    float band_linear[FFT_BANDS] = {0};
    int bins_count[FFT_BANDS] = {0};

    for (int i = 1; i < FFT_SIZE / 2; i++) {
        float f = i * bin_hz;
        for (int b = 0; b < FFT_BANDS; b++) {
            if (f >= band_edges[b] && f < band_edges[b + 1]) {
                band_sum[b] += magnitude[i];
                band_energy[b] += magnitude[i] * magnitude[i];
                band_peak[b] = max(band_peak[b], magnitude[i]);
                bins_count[b]++;
                break;
            }
        }
    ...
    // Усреднение по каждой полосе
    for (int b = 0; b < FFT_BANDS; b++) {
        if (bins_count[b] > 0) {
            if (spectrum_profile == 1) {
                band_linear[b] = band_peak[b];
            } else if (spectrum_profile == 2) {
                band_linear[b] = band_sum[b] / bins_count[b];
            } else {
                band_linear[b] = sqrtf(band_energy[b] / bins_count[b]);
            }
        }
        // Ограничение от перегрузки
        if (band_linear[b] > 1.0f) band_linear[b] = 1.0f;
    }

    // ========== 5. НЕЗАВИСИМАЯ НОРМАЛИЗАЦИЯ ДЛЯ КАЖДОЙ ПОЛОСЫ ==========
    // КЛЮЧЕВОЕ ИЗМЕНЕНИЕ: каждая полоса нормализуется сама по себе!
    
    const float TARGET_RMS_DB = -18.0f;   // Целевой уровень для каждой полосы
    const float MAX_GAIN_DB = 12.0f;      // Максимальное усиление (+12 dB)
    const float MIN_GAIN_DB = -6.0f;      // Минимальное усиление (-6 dB, не убавляем слишком сильно)
    const float RMS_HYSTERESIS = 6.0f;    // Гистерезис для предотвращения дерганья
    
    static float band_gain_db[FFT_BANDS] = {0};  // Текущее усиление для каждой полосы
    static float band_rms_db[FFT_BANDS] = {-60}; // Текущий RMS для каждой полосы

    const float ATTACK_RMS = 0.8f;   // Быстрый attack (80% изменения за кадр)
    const float RELEASE_RMS = 0.3f;  // Медленный release (30% изменения)

    for (int b = 0; b < FFT_BANDS; b++) {
        // 5.1 Текущий уровень полосы в dB
        static bool band_rms_initialized = false;
        static uint8_t band_rms_profile = 255;
        if (!band_rms_initialized || band_rms_profile != spectrum_profile) {
            std::fill(std::begin(band_rms_db), std::end(band_rms_db), -60.0f);
            std::fill(std::begin(band_gain_db), std::end(band_gain_db), 0.0f);
            band_rms_initialized = true;
            band_rms_profile = spectrum_profile;
        }

        float current_db = 20.0f * log10f(band_linear[b] + 1e-6f);
        
        // 5.2 Сглаживание RMS (свой для каждой полосы)
        if (current_db > band_rms_db[b]) {
            band_rms_db[b] = band_rms_db[b] + ATTACK_RMS * (current_db - band_rms_db[b]);
        } else {
            band_rms_db[b] = band_rms_db[b] + RELEASE_RMS * (current_db - band_rms_db[b]);
        }
        
        // 5.3 Вычисляем необходимое усиление для достижения цели
        const float profile_target_db = spectrum_profile == 1 ? -22.0f : (spectrum_profile == 2 ? -20.0f : TARGET_RMS_DB);
        const float profile_min_gain_db = spectrum_profile == 1 ? -9.0f : MIN_GAIN_DB;
        const float profile_max_gain_db = spectrum_profile == 1 ? 9.0f : (spectrum_profile == 2 ? 15.0f : MAX_GAIN_DB);
        float target_gain = profile_target_db - band_rms_db[b];
        target_gain = std::clamp(target_gain, profile_min_gain_db, profile_max_gain_db);
        
        // 5.4 Сглаживание усиления (плавные переходы)
        if (target_gain > band_gain_db[b]) {
            band_gain_db[b] = band_gain_db[b] + 0.5f * (target_gain - band_gain_db[b]);
        } else {
            band_gain_db[b] = band_gain_db[b] + 0.2f * (target_gain - band_gain_db[b]);
        }
        
        // 5.5 Применяем усиление к текущей амплитуде
        float gained_linear = band_linear[b] * powf(10.0f, band_gain_db[b] / 20.0f);
        gained_linear = std::clamp(gained_linear, 0.0f, 1.0f);
        
        // 5.6 Нелинейная коррекция (опционально, для лучшего вида)
        gained_linear = powf(gained_linear, spectrum_profile == 1 ? 0.78f : (spectrum_profile == 2 ? 0.92f : 0.85f));
        
        // ========== 6. Преобразование в байты для вывода ==========
        m_fft_items.spectrum[b] = (uint8_t)(gained_linear * 255.0f);
    }
}
// —————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
...
void Audio::IIR_calculateCoefficients() { // Infinite Impulse Response (IIR) filters
    ...
    // float total_boost_db = fmax(fmax(fmax(0, m_audio_items.gain_ls_db), m_audio_items.gain_peq_db), m_audio_items.gain_hs_db);
    m_audio_items.pre_gain = 1.0f;   // Максимальная громкость, никакого автоматического снижения
