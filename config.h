...
#define TFT_DC   9
#define TFT_CS   10
#define TFT_MOSI 11 /* can be SDA, DIN, SDI */
#define TFT_SCLK 12 /* can be SCL, SCK, CLK */
#define TFT_RST  13
#define TFT_BL   14
...
//    Долгое нажатие = плейлист
//
// 2) Режим двух энкодеров: укажите реальные контакты A/B для NAV-энкодера.
//    Энкодер 1 = громкость + кнопка play/pause
//    Энкодер 2 = навигация по станциям + кнопка смены экрана + Долгое нажатие = плейлист



// === ENCODER PULL-UP SETTINGS ===
// true  - use internal INPUT_PULLUP for encoder pins
...
// === IR REMOTE DEFAULTS ===
// Runtime settings are stored in LittleFS as /ir_remote_config.json.
// Set the default IR receiver GPIO here. Leave 255 to disable IR control.
#define IR_REMOTE_PIN 255

// Common 21-key NEC/Panasonic remote defaults.
// These are used only when /ir_remote_config.json does not exist yet.
...
// === FONT CONFIG ===
#define CLOCK_FONT Fallout_54px
...
// Set ENABLE_STATION_SELECT_DEBOUNCE to 0 to disable debounce and connect immediately.
#define ENABLE_STATION_SELECT_DEBOUNCE 1
// Delay in milliseconds before performing actual stream connect after selection
...
// === DEBUG ===
// === DISPLAY OPTIONS ===
#define SHOW_SECONDS true  // true - показывать секунды, false - только часы и минуты
